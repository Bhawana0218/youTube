...
export const requestDownload = async (req, res) => {
  try {
    ...
    if (!isPremium) {
      ...
      if (todayCount >= FREE_DAILY_LIMIT) {
        return res.status(403).json({
          message: "Free download limit reached. Upgrade to Premium for unlimited downloads.",
    ...
    return res.status(200).json({
      ...
      downloadUrl: `/uploads/${String(video.filepath || "").split(/[\\/]/).pop()}`,
      fileName: video.videotitle || "video",
      isPremium,
      remainingToday: isPremium ? null : Math.max(FREE_DAILY_LIMIT - todayCountAfter, 0),
    });
  } catch (error) {
    console.log("Download request error:", error);
    return res.status(500).json({
      message: "Something went wrong while processing download.",
    });
  }
};
